/**
 * Google Sheets adapter utilities.
 *
...
/** Parse a spreadsheet ID from a full URL or return as-is */
export function parseSheetId(input: string): string {
  const m = input.match(/\/spreadsheets\/d\/([a-zA-Z0-9_-]+)/);
  if (m) return m[1];
  if (/^[a-zA-Z0-9_-]{20,}$/.test(input)) return input;
  throw new CliError('ARGUMENT', 'Invalid spreadsheet ID', 'Provide a sheet ID or Google Sheets URL');
}
